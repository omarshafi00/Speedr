...
//
//  Structure (notches at 20% from left):
//      ──────┬─────────────────────────────────
//            │  (8pt notch)
//
...
/// Notches are positioned at 20% from the left of the total line width
...
/// Individual focal line (top or bottom) with notch at 20% from left
struct FocalLineView: View {
    ...
    /// Notch positioned at 20% from the left
    private var notchWithOffset: some View {
        HStack(spacing: 0) {
            // Left spacer (20% of total width minus half the notch width)
            Spacer()
...
struct FocalPointConfig {
    ...
    /// Notch position: 20% from the left of the total width
